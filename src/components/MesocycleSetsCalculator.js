...
const MesocycleSetsCalculator = () => {
  ...
  const calculateWeeklyIncrease = useMemo(() => {
  ...
  const [startingSets] = useState(MEV);
  const [weeklyIncrease] = useState(calculateWeeklyIncrease);
  const [deloadPercentage] = useState(50);

  const calculateSets = useMemo(() => {
    const mesocycle = [];
    ...
    return mesocycle;
  }, [startingSets, weeklyIncrease, deloadPercentage]);
  ...
  <div className={styles.container}>
    ...
    <div className={styles.tableContainer}>
      <table className={styles.table}>
        ...
        <tbody>
          {calculateSets.map((weekSets, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
